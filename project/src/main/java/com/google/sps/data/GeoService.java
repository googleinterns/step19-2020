...
public class GeoService {
    ...
    private List<String> validCountryList = Arrays.asList("AR", "AU", "AT", "BE", "BR", "CA", "CL", "CO", 
                                                          "CZ", "DK", "EG", "FI", "FR", "DE", "GR", "HK", 
                                                          "HU", "IN", "ID", "IE", "IL", "IT", "JP", "KE", 
                                                          "KR", "MY", "MX", "NL", "NZ", "NG", "NO", "PH", 
                                                          "PL", "PT", "RO", "RU", "SA", "SG", "ZA", "SE", 
                                                          "CH", "TW", "TH", "TR", "UA", "GB", "US", "VN"); 

    private final OkHttpClient httpClient = new OkHttpClient();

    private String API_KEY = "Please insert the actual API Key (found in our project console) when you pull.";

    // This function can be used to get all the countries which Google has trend data on. It has a runtime of around 20s
    // and since valid countries are unlikely to change soon, the validCountryList can be used in place.
    public List<String> getValidCountries() throws MalformedURLException, IOException {
      ...
      try {
        ...
        URL countryCodes = new URL("https://pkgstore.datahub.io/core/country-list/data_json/data/8c458f2d15d9f2119654b29ede6e45b8/data_json.json");
    ...
    // This function goes through each country and checks if it has an accessible Google trend RSS feed. It returns
    ...
    private List<String> checkValidity(JsonArray a) {
      try {
        ...
        for (Object o : a) {
          ...
          String url = String.format("https://trends.google.com/trends/trendingsearches/daily/rss?geo=%s", country);
    ...
    public JsonObject getUserCoordinates() throws IOException {
      ...
      Request request = new Request.Builder()
        .url(String.format("https://www.googleapis.com/geolocation/v1/geolocate?key=%s", API_KEY))
    ...
    // This function takes in a set of coordinates and sends a request to the Geocoding API. The function returns the country
    // code of those coordinates.
    public String getUserCountry(JsonObject coordinates) throws IOException {
      String url = String.format("https://maps.googleapis.com/maps/api/geocode/json?latlng=%s,%s&key=%s", coordinates.get("lat").getAsString(), coordinates.get("lng").getAsString(), API_KEY);
      Request request = new Request.Builder()
        .url(url)
        .build();
      try (Response response = httpClient.newCall(request).execute()) {
        ...
        // Get response body
        JsonArray results = new Gson().fromJson(response.body().string(), JsonObject.class).get("results").getAsJsonArray();
        JsonObject countryObject = results.get(results.size()-1).getAsJsonObject();
        JsonObject countryCodeObject = countryObject.get("address_components").getAsJsonArray().get(0).getAsJsonObject();
